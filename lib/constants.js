...
export function buildWhatsAppOrderLink(productName, details) {
  let message = `Hi Zenvora Mart BD! I want to order: ${productName}.`;

  if (details && details.price) {
    message += ` Price: ৳${details.price}.`;
    if (details.couponCode && details.discount) {
      message += ` Coupon applied: ${details.couponCode} (-৳${details.discount}). Final price: ৳${details.finalPrice}.`;
    }
  }

  message += ` Please confirm price & availability.`;
